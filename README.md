...
5. 로그인 페이지에서 기본 계정(아래 "🔐 로그인 정보" 참조)으로 로그인합니다.
...
# 4. 브라우저에서 http://localhost:5000 접속 후 로그인
# (아래 "🔐 로그인 정보" 섹션 참조)
```

## 🔐 로그인 정보

서버 실행 후 브라우저에서 `http://localhost:5000` 접속 시 로그인 페이지가 나타납니다.

**기본 계정:**
- **아이디**: `malgeun_admin`
- **비밀번호**: `Tube2024!@Secure`

> ⚠️ **보안 권장사항**: 프로덕션 환경에서 사용할 경우 반드시 `app.py`의 계정 정보를 변경하세요.
